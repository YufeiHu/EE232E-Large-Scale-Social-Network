...
for(i in unique(sector)){
    #find out all the nodes belong to a sector
    node_curr = nodes[which(sector == i)]
    sector_sum = 0

    #for each node belongs to this sector
    for(j in node_curr){
        #find its neighbors
        neighbor_curr = as.vector(neighbors(g_mst, j, mode = "all"))
        node_name = which(j == nodes)
        #definition 1
        curr = length(which(sector[neighbor_curr] == sector[node_name])) / length(neighbor_curr)
        sector_sum = sector_sum + curr
    }

    alpha1 = c(alpha1, sector_sum / length(nodes))

    #definition 2
    alpha2 = c(alpha2, length(node_curr) / length(nodes))
}
